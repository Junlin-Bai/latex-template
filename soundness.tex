  ...
  \end{definition}
    ...
    \begin{definition}[Resolution and Filter-Indexed Views]
      ...
      \begin{definition}[Static Admissibility]
        ...
        conditions, filter length conditions, fixed-index validity, and dimension
        compatibility.
...
\]
  ...
  \begin{definition}[Assertion Language and Semantics]
    Single-side view expressions are generated by
      ...
      \equiv_{(z,z')}^{D}
      \mid
      \Phi_{\mathcal V_0}\land\Psi_{\mathcal V_1}
      \mid
      \Phi_{\mathcal V_0}\lor\Psi_{\mathcal V_1}
      \mid
      \neg\Phi_{\mathcal V_0}
      \mid
      \Phi_{\mathcal V_0} * C_{\mathcal U}
      \mid
      \operatorname{proj}\!\left(
      \operatorname{Tr}_{\mathcal U}(\Phi_{\mathcal V_0})
      \right).
      \]
    
    For current descriptor stores \((\xi,\xi')\), an assertion denotes a local
    projector
    \[
    \llbracket \Phi_{\mathcal V}\rrbracket_{\xi,\xi'}
    \]
    on the resolved left/right physical register memories. If \(\mathcal V\) is
    not well formed under \((\xi,\xi')\), then
    \[
    \llbracket \Phi_{\mathcal V}\rrbracket_{\xi,\xi'}
    =
    0_{\mathsf{Reg}^{L,R}}.
    ...
    For primitive view-indexed assertions,
    \[
    ...
    Descriptor-only assertions are scalar local assertions:
    \[
...
\]
    ...
    For one-sided filter support,
\[
\llbracket \Pi_{f}^{N}\rrbracket_{\xi,\xi'}
=
\begin{cases}
I_{\mathsf{Reg}^{L,R}},
&
...
and
\[
\llbracket \Pi_{f}^{m,N}\rrbracket_{\xi,\xi'}
=
\begin{cases}
I_{\mathsf{Reg}^{L,R}},
&
...
\]
    ...
    \equiv_{(g^{f},g'^{f'})}
    \]
    is ordinary equality on the resolved ordered views when
    \[
    \operatorname{desc}_{\xi}(g)=(\ell_g,\alpha_g),
    \qquad
    \operatorname{desc}_{\xi'}(g')=(\ell'_{g'},\alpha'_{g'}),
    ...
    \]
    is well formed only when the auxiliary resource contexts certify that
...
\begin{definition}[Footprint Equality]
  ...
  \begin{definition}[Descriptor Dependencies of View Indices]
    ...
    For paired and packaged views, dependencies are collected componentwise. For
    a local assertion,
    \[
