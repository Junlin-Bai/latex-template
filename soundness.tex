...
\begin{definition}[Variable Names to Address Indexes]
  ...
  \mathsf{Classical}.
  \]
  A base register variable such as
  \[
  \bar q,
  \qquad
  p,
  ...
  x
  \]
  is also a logical list name of the corresponding sort, with an initial
  descriptor view over its declared physical address list.
...
\begin{definition}[Logical Lists, Length, and Descriptor Views]
  The physical register memories are fixed address-indexed memories. Descriptor
...
\begin{definition}[Descriptor Stores and Register States]
  ...
  All code linear algebra is over \(\mathbb F_2\).  A code matrix \(C_m\)
determines a fixed non-empty binary linear code
\[
\mathsf{Code}_n(C_m)\le \mathbb F_2^n .
\]
...
\rho_{\xi,c}^{\mathsf Q}.
\]
Witnesses satisfy the analogous joint block-diagonal condition.
  
  For semantic branch notation, write
  \[
...
\]
      ...
      (g|_{F=0},g|_{F=1}),
      \]
      with relative order preserved inside each fiber. Thus, when
      \[
      \operatorname{desc}_{\xi}(g)=(\ell_g,\alpha_g),
      \]
      \[
      ...
      The notations \(u^{f}\) and \(u^{\uparrow f}\) extend componentwise to
parenthesized packages of views.
...
It checks descriptor and register obligations, including displayed length
conditions, filter length conditions, fixed-index validity, dimension
compatibility, and the linear-type obligations of liveness, sorting, and no
physical aliasing in multi-system uses.
...
\begin{definition}[Program Semantics]
  In this definition, \(\sem S\) is used only for programs satisfying the
  static admissibility judgment
  \[
  \Gamma\vdash_{\mathsf{wf}} S.
  \]
  Ill-formed programs have no denotation.
  ...
  \operatorname{desc}_{\xi}(g)=(\ell_g,\alpha_g).
  \]
  Static admissibility guarantees
  \[
  e=\ell_g=N,
  \qquad
  F\in\{0,1\}^{e}.
  \]
  Let \(D_f(\xi)\) be the descriptor store obtained from \(\xi\) by setting
  \[
  ...
  =
  \sum_{\xi}
  \ket{D_f(\xi)}\bra{D_f(\xi)}_{\mathsf D}
  \otimes
  \Delta[\xi].
  \]
  ...
  \]
  Let \(D_f(\xi)\) be the descriptor store obtained from \(\xi\) by setting
  \(g\) to the unique descriptor list \(h=(e,\alpha_h)\) satisfying
  \[
  \alpha_h(t)=
  \begin{cases}
...
For a classical expression \(E\), let \(\Pi_{E=v,\xi}\) be the branchwise
...
Then
\[
\mathcal E^{\bar q[i]:=\ket0}_{\xi}(\rho)
=
\ket0_{a_q}\bra0
\otimes
\operatorname{Tr}_{a_q}(\rho).
\]
...
Then
\[
\mathcal E^{\bar q[i]\asteq U}_{\xi}(\rho)
=
U_{a_q}\rho U_{a_q}^{\dagger}.
\]
...
Then
\[
\mathcal E^{x[i]:=\mathcal M[\bar q[j]]}_{\xi}(\rho)
=
\sum_{r\in[m]}
\ket r_{a_x}\bra r
\otimes
M^{a_q}_r
\operatorname{Tr}_{a_x}(\rho)
M^{a_q\dagger}_r .
\]
...
The register effect is
\[
\mathcal E^{p:=_{\$}\textit{rand}_{n}(C_m)}_{\xi}(\rho)
=
...
\]
  ...
  \begin{definition}[Assertion Language and Semantics]
...
Equality on packages follows the same sort-sensitive convention.
    ...
    Register assertions are generated by
    \[
    R_{\mathcal V} ::=
    A_{\mathcal V}
    \mid
...
is pointwise equality over all pairs in \(\mathcal V\).
    ...
    Descriptor formulas denote classical truth values:
    \[
    \llbracket \chi\rrbracket^D_{\xi,\xi'}
    \in
    \{0,1\}.
    \]
    They are interpreted by
    \[
    \llbracket \top_D\rrbracket^D_{\xi,\xi'}=1,
    \qquad
    \llbracket \bot_D\rrbracket^D_{\xi,\xi'}=0,
    \]
    \[
    \llbracket \equiv_{(f,f')}\rrbracket^D_{\xi,\xi'}=1
    \quad\Longleftrightarrow\quad
    \xi(f)=\xi'(f'),
    \]
    \[
    \llbracket \equiv_{(\iota,\iota')}\rrbracket^D_{\xi,\xi'}=1
    \quad\Longleftrightarrow\quad
    \xi(\iota)=\xi'(\iota'),
    \]
    \[
    \llbracket \Pi_f^N\rrbracket^D_{\xi,\xi'}=1
    \quad\Longleftrightarrow\quad
    \xi(f)\in\{0,1\}^N,
    \]
    \[
    \llbracket \Pi_f^{m,N}\rrbracket^D_{\xi,\xi'}=1
    \quad\Longleftrightarrow\quad
    \xi(f)\in\mathsf{Filt}(m,N),
    \]
    with Boolean connectives interpreted classically.
    
    The substitution
    \[
    \mathsf{sub}_{\xi,\xi'}(R_{\mathcal V})
    \]
    replaces every displayed logical view in \(R_{\mathcal V}\) by its resolved
    physical view under \((\xi,\xi')\). It returns a physical-register assertion
    expression. If the displayed views in \(R_{\mathcal V}\) are not well formed under
    \((\xi,\xi')\), then
    \[
    \llbracket R_{\mathcal V}\rrbracket^R_{\xi,\xi'}=0.
    \]
    
    Register assertions denote physical-register projectors:
    \[
    \llbracket R_{\mathcal V}\rrbracket^R_{\xi,\xi'}
    \in
    \operatorname{Proj}
    \left(
    ...
    \right).
    \]
    They are interpreted after substitution. In particular,
    \[
    \llbracket A_{\mathcal V}\rrbracket^R_{\xi,\xi'}
    =
    A_{\mathsf{phys}_{\xi,\xi'}(\mathcal V)}.
    \]
    
    For finite-support \(\mu\) and finite domain \(D\),
    \[
    \Pi_z^\mu
    \stackrel{\triangle}{=}
    ...
    For probability-register entries,
    \[
    \equiv^{\mu}_{(p[i],p'[j])}
    \stackrel{\triangle}{=}
    ...
    \stackrel{\triangle}{=}
    \sum_{c\in\mathsf{Code}_n(C_m)}
    \ket c_{p[1,\ldots,n]}\bra c .
    \]

    For quantum logical references,
    \[
    ...
    \]
    on the resolved systems, extended by identity.
    
    List equality is sort-sensitive. Classical and probability lists use pointwise
    equality. Quantum lists use block-swap equality on the resolved ordered blocks:
    \[
    \equiv^{\mathsf q}_{(Q,Q')}
    \stackrel{\triangle}{=}
    \Pi_{\operatorname{Fix}(\mathsf{SWAP}_{Q,Q'})}
    =
    \frac12
    \left(
    I+\mathsf{SWAP}_{Q,Q'}
    ...
    \equiv_{(g[t],g'[t])},
    &
    \operatorname{sort}(g)\in\{\mathsf{Classical},\mathsf{Probability}\},
    \\[2ex]
    \equiv^{\mathsf q}_{(\mathsf{phys}_{\xi}(g),\mathsf{phys}_{\xi'}(g'))},
    &
    \operatorname{sort}(g)=\mathsf{Quantum}.
    \end{cases}
    \]
    Otherwise its register interpretation is \(0\).
    
    For fixed-length list equality, if \(n=n'\), then
    \[
    ...
    \end{cases}
    \]
    If \(n\neq n'\), its register interpretation is \(0\).

    For filter-indexed view equality, suppose
    \[
    \xi(f)=F,
    \qquad
    \xi'(f')=F'.
    ...
    \equiv_{(g^{f},g'^{f'})}
    \]
    is the corresponding sort-sensitive equality on the resolved ordered views when
    \[
    \operatorname{desc}_{\xi}(g)=(\ell_g,\alpha_g),
    \qquad
    \operatorname{desc}_{\xi'}(g')=(\ell'_{g'},\alpha'_{g'}),
    ...
    General assertions denote local projectors:
    \[
    ...
    \right).
    \]
    Descriptor formulas are embedded by
    \[
    \llbracket \chi\rrbracket_{\xi,\xi'}
    =
    \begin{cases}
    I_{\mathsf{Reg}^{L,R}},
    &
    \llbracket \chi\rrbracket^D_{\xi,\xi'}=1,
    \\[0.4ex]
    0_{\mathsf{Reg}^{L,R}},
    &
    \llbracket \chi\rrbracket^D_{\xi,\xi'}=0.
    \end{cases}
    \]
    Register assertions are embedded by
    \[
    \llbracket R_{\mathcal V}\rrbracket_{\xi,\xi'}
    =
    \llbracket R_{\mathcal V}\rrbracket^R_{\xi,\xi'}.
    \]
    
    For projective assertions \(\Phi,\Psi\), define branchwise:
    \[
    ...
    \right).
    \]
    It traces only register payload systems.
    ...
    \begin{definition}[Approximate Classical-Quantum Lifting]
      ...
      The parameter \(\delta\) bounds only postcondition-violating mass under exact
marginals.
...
\llbracket\Phi\rrbracket_{\xi,\xi'}
\le
\llbracket\Psi\rrbracket_{\xi,\xi'} .
\]
\end{definition}
...
\begin{definition}[Footprint Equality]
  ...
  \begin{definition}[Descriptor Dependencies]
    ...
    \[
    \operatorname{ddep}(\top_D)
    =
    \operatorname{ddep}(\bot_D)
    =
    \emptyset,
    \]
    \[
